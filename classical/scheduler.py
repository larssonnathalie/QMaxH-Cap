...
#from .gurobi_model import create_gurobi_model  REMOVE HASHTAG
from .data_handler import load_data_from_intermediate
import pandas as pd

def solve_and_save_results(solver_type="z3", source="intermediate", lambdas=None):
    """
...
def schedule_dict_to_df(schedule, shifts_df):
    ...
    for p, shift_list in schedule.items():
        pid = int(p.replace('physician', '')) if str(p).startswith('physician') else p
        for s in shift_list:
